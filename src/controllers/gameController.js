...
// Send invitation using username
exports.sendInvitation = async (req, res) => {
  try {
    const { toUsername, gameId } = req.body
    ...
    if (existing) {
      return res.redirect('/dashboard') // Already invited
    }
...
exports.acceptInvitation = async (req, res) => {
  try {
    ...
    if (!invitation || invitation.toUserId.toString() !== userId) {
    ...
    if (game && user && !game.players.some(p => p.userId.toString() === userId)) {
      game.players.push({
        userId,
        username: user.username,
        role: 'civilian',
